...
int fobos_rx_get_api_info(char *lib_version, char *drv_version)
{
    if (lib_version)
    {
        strcpy(lib_version, LIB_VERSION " "__DATE__
                                        " "__TIME__);
    }
    if (drv_version)
    {
...
//==============================================================================
void fobos_i2c_transfer(struct fobos_dev_t *dev, uint8_t address, uint8_t *tx_data, uint16_t tx_size, uint8_t *rx_data,
                        uint16_t rx_size)
{
    ...
    if (result == 0)
    {
        ...
        {
            xsize =
                libusb_control_transfer(dev->libusb_devh, CTRLO, req_code, address, 0, tx_data, tx_size, CTRL_TIMEOUT);
            if (xsize != tx_size)
            {
                result = FOBOS_ERR_CONTROL;
            }
        ...
        {
            xsize =
                libusb_control_transfer(dev->libusb_devh, CTRLI, req_code, address, 0, rx_data, rx_size, CTRL_TIMEOUT);
            if (xsize != tx_size)
            {
                result = FOBOS_ERR_CONTROL;
            }
